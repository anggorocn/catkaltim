  ...
  ***/
  ...
  class Kelautan extends Entity{ 
    ...
    **/
	...
	function selectByParamsSatuanKerja($paramsArray=array(),$limit=-1,$from=-1, $statement='', $sOrder="order by A.satker_id asc")
	{
		$str = "
	...
	function selectByParamsMonitoringPegawai($paramsArray=array(),$limit=-1,$from=-1, $statement='', $orderby='', $jadwalTes)
	{
		$str = "
			SELECT 
			...
			--left join jadwal_tes_simulasi_pegawai e on a.pegawai_id = e.pegawai_id and jadwal_tes_id=".$jadwalTes."
			WHERE 1=1
	 		"; 
	...
	function selectByParamsMonitoringPenilaianPegawai($paramsArray=array(),$limit=-1,$from=-1, $statement='', $orderby="ORDER BY COALESCE(A.LAST_PANGKAT_ID,0) DESC, COALESCE(A.LAST_ESELON_ID,'99') ASC", $reqTahun='2015')
	{
		$str = "
			...
			, P.JT_TANGGAL_TES, P.JT_ACARA, P.JT_KETERANGAN, p.JADWAL_AWAL_TES_ID
			FROM ".$this->db.".pegawai A
			LEFT JOIN ".$this->db.".pangkat B ON A.LAST_PANGKAT_ID = B.PANGKAT_ID
			LEFT JOIN ".$this->db.".eselon C ON A.LAST_ESELON_ID = C.ESELON_ID
			LEFT JOIN ".$this->db.".satker D ON A.SATKER_ID = D.SATKER_ID
	...
	function selectByParamsMonitoringTableTalentPoolPotensiKompetensiMonitoring($paramsArray=array(),$limit=-1,$from=-1, $statement='', $orderby='', $reqTahun='2015', $searcJson= "")
	{
		$str = "
			...
			(
				...
				'' JPM_TOTAL, '' IKK_TOTAL,
				A.*,
				A.FORMULA_ID FORMULA_ID_ASSESMENT,
				A.FORMULA FORMULA_NAMA
				FROM
				(
					SELECT A.PEGAWAI_ID, A.NAMA, A.LAST_JABATAN NAMA_JAB_STRUKTURAL,
					X.FORMULA_ID,
					X.FORMULA,
					ROUND(COALESCE(X.NILAI,0),2) NILAI_X,
					ROUND(COALESCE(Y.NILAI,0),2) NILAI_Y,
					CAST
							(
								CASE WHEN
					...
					(
						...
						INNER JOIN FORMULA_ESELON F ON E.FORMULA_ESELON_ID = F.FORMULA_ESELON_ID
						INNER JOIN FORMULA_ASSESMENT G ON F.FORMULA_ID = G.FORMULA_ID 
						WHERE 1=1 AND D.ASPEK_ID = '2'
						AND TO_CHAR(D.TANGGAL_TES, 'YYYY') = '".$reqTahun."'
					) X ON A.PEGAWAI_ID = X.PEGAWAI_ID
					INNER JOIN
   ...
   function selectByParamsMonitoringTableTalentPoolJPMMonitoring($paramsArray=array(),$limit=-1,$from=-1, $statement='', $orderby='', $reqTahun='2015', $searcJson= "")
	{
		$str = "
			...
			(
				...
				(
					SELECT A.PEGAWAI_ID, A.NAMA, A.LAST_JABATAN NAMA_JAB_STRUKTURAL,
					COALESCE(X.NILAI,0) NILAI_X,
					COALESCE(Y.NILAI,0) NILAI_Y,
					CAST
							(
								CASE WHEN
								COALESCE(X.NILAI,0) <= KD_X.KUADRAN_X1 
   ...
   function selectByParamsMonitoringTableTalentPoolJPMGrafik($paramsArray=array(),$limit=-1,$from=-1, $statement='', $orderby='', $reqTahun='2015')
	{
		$str = "
			SELECT
			...
			(
				...
				FROM
				(
					SELECT A.PEGAWAI_ID, X.JADWAL_TES_ID, A.NAMA, A.LAST_JABATAN NAMA_JAB_STRUKTURAL,
					...
					(
						...
						INNER JOIN FORMULA_ESELON C ON B.FORMULA_ESELON_ID = C.FORMULA_ESELON_ID
						INNER JOIN FORMULA_ASSESMENT D ON C.FORMULA_ID = D.FORMULA_ID 
						WHERE 1=1 AND TO_CHAR(A.TANGGAL_TES, 'YYYY') = '".$reqTahun."' 
						AND ASPEK_ID in (1,2) 
    ...
    function getCountByParamsMonitoringTableTalentPoolJPMMonitoring($paramsArray=array(), $statement="", $reqTahun='', $searcJson= "")
	{
		$str = "
				SELECT COUNT(1) ROWCOUNT
				FROM
						...
						(
							...
							(
								SELECT
									...
									, E.FORMULA_ID
								FROM penilaian D
								INNER JOIN JADWAL_TES B ON D.JADWAL_TES_ID = B.JADWAL_TES_ID
								INNER JOIN FORMULA_ESELON C ON B.FORMULA_ESELON_ID = C.FORMULA_ESELON_ID
								INNER JOIN FORMULA_ASSESMENT E ON C.FORMULA_ID = E.FORMULA_ID 
								INNER JOIN 
								(
									SELECT A.PEGAWAI_ID ID, '' ESSELON, A.NAMA, A.LAST_JABATAN NAMA_JAB_STRUKTURAL, A.SATKER_ID KODE_UNKER
									FROM ".$this->db.".pegawai A
								...
								AND TO_CHAR(D.TANGGAL_TES, 'YYYY') = '".$reqTahun."'
								GROUP BY D.PEGAWAI_ID, TO_CHAR(D.TANGGAL_TES, 'YYYY'), E.FORMULA
									, E.FORMULA_ID
							) X ON A.PEGAWAI_ID = X.PEGAWAI_ID
							INNER JOIN
							(
	...
	function getCountByParamsMonitoringPegawai($paramsArray=array(), $statement="", $jadwalTes)
	{
		$str = "
				SELECT COUNT(1) ROWCOUNT
				FROM ".$this->db.".pegawai A
    ...
    function selectByParamsMonitoringPegawaiEks($paramsArray=array(),$limit=-1,$from=-1, $statement='', $orderby='')
	{
		$str = "
			SELECT  
			A.PEGAWAI_ID IDPEG
			, case when status_jenis = 2 then 'PNS Luar Prov Bali'
				when status_jenis = 3 then 'Kalangan Umum'
				else 'PNS Prov Bali' end status
			, A.NIK, A.NIP_BARU
 			, A.NAMA
 			, JENIS_KELAMIN, A.TEMPAT_LAHIR, A.TGL_LAHIR 			 			
			, tempat_kerja
			, email
			, hp
			, A.SATKER_EKSTERNAL_ID
			, B.NAMA SATKER_EKSTERNAL
			, a.LAST_JABATAN
			, a.JABATAN_LAMAR
			FROM ".$this->db.".pegawai A
			LEFT JOIN ".$this->db.".satker_eksternal B ON A.SATKER_EKSTERNAL_ID = B.SATKER_EKSTERNAL_ID
    ...
    function getCountByParamsMonitoringPegawaiEks($paramsArray=array(), $statement="")
	{
		$str = "
				SELECT COUNT(1) ROWCOUNT
				FROM ".$this->db.".pegawai A
  ...
  function selectByParamsMonitoringAdminKegiatan($paramsArray=array(),$limit=-1,$from=-1, $statement='', $orderby='ORDER BY nama ASC')
	{
		$str = "
			SELECT a.user_login, a.nama, b.asesor_id from user_app a
			left join asesor b on a.pegawai_id =b.asesor_id
