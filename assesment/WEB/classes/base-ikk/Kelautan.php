  ...
  class Kelautan extends Entity{ 
    ...
    **/
    ...
    function insertpegawai()
	{
		...
		(
			PEGAWAI_ID, NIP_BARU, SATKER_ID, STATUS_JENIS, NIK, STATUS_PEGAWAI_ID, NAMA
			, TEMPAT_LAHIR, TGL_LAHIR, JENIS_KELAMIN, AGAMA, STATUS_KAWIN
			, ALAMAT, ALAMAT_TEMPAT_KERJA, EMAIL, SOSIAL_MEDIA, HP, AUTO_ANAMNESA
			, LAST_PANGKAT_ID, LAST_TMT_PANGKAT, LAST_ESELON_ID, LAST_JABATAN, LAST_TMT_JABATAN
			, LAST_DIK_JENJANG, LAST_DIK_JURUSAN, TEMPAT_KERJA,SATKER_EKSTERNAL_ID,JABATAN_LAMAR
		) 
		VALUES 
		(
			...
			, '".$this->getField("ALAMAT_TEMPAT_KERJA")."'
			, '".$this->getField("EMAIL")."'
			, '".$this->getField("SOSIAL_MEDIA")."'
			, '".$this->getField("HP")."'
			, '".$this->getField("AUTO_ANAMNESA")."'
			...
			, '".$this->getField("TEMPAT_KERJA")."'
			, '".$this->getField("SATKER_EKSTERNAL_ID")."'
   ...
   function insertpegawaiEksport()
    ...
    function updatepegawai()
	{
		...
		, JABATAN_LAMAR= '".$this->getField("JABATAN_LAMAR")."'
   ...
   function selectByParamsMonitoringTableTalentPoolJPMGrafik($paramsArray=array(),$limit=-1,$from=-1, $statement='', $orderby='', $reqTahun='2015')
	...
	function selectByParamsPegawai($paramsArray=array(),$limit=-1,$from=-1, $statement='', $orderby='')
	{
		...
		$str = "
			SELECT 
			A.SATKER_ID KODE_UNKER,
			A.SATKER_EKSTERNAL_ID KODE_EKSTERNAL,
 			A.PEGAWAI_ID IDPEG, A.NIP NIP_LAMA, A.NIP_BARU, A.NAMA
 			, '' GELAR_DEPAN, '' GELAR_BELAKANG, JENIS_KELAMIN, A.TEMPAT_LAHIR, A.TGL_LAHIR, case A.status_pegawai_id when 1 then 'CPNS' when 2 then 'PNS' when 3 then 'Pensiun' else '' end STATUS
 			, B.KODE NAMA_GOL, A.LAST_TMT_PANGKAT TMT_GOL_AKHIR, C.NAMA NAMA_ESELON, A.LAST_JABATAN NAMA_JAB_STRUKTURAL, A.LAST_TMT_JABATAN TMT_JABATAN, '' TELP
 			, '' STATUS_KANDIDAT, '' UMUR, E.NAMA PENDIDIKAN_NAMA, A.LAST_DIK_JURUSAN
 			...
 			, A.STATUS_JENIS
 			, A.STATUS_PEGAWAI_ID, A.STATUS_JENIS, A.LAST_PANGKAT_ID, A.LAST_ESELON_ID, A.LAST_DIK_JENJANG, A.TEMPAT_KERJA
 			, F.NAMA SATKER_EKSTERNAL
 			, A.JABATAN_LAMAR
			FROM ".$this->db.".pegawai A
			LEFT JOIN ".$this->db.".pangkat B ON A.LAST_PANGKAT_ID = B.PANGKAT_ID
			LEFT JOIN ".$this->db.".eselon C ON A.LAST_ESELON_ID = C.ESELON_ID
			LEFT JOIN ".$this->db.".satker D ON A.SATKER_ID = D.SATKER_ID
			LEFT JOIN ".$this->db.".pendidikan E ON A.LAST_DIK_JENJANG = E.PENDIDIKAN_ID
			LEFT JOIN ".$this->db.".satker_eksternal F ON A.SATKER_EKSTERNAL_ID = F.SATKER_EKSTERNAL_ID
	...
	{
		$str = "SELECT riwayat_pendidikan_id, a.nama_sekolah, a.tahun_awal, a.tahun_akhir,a.tempat,
			   PEGAWAI_ID,
			   a.PENDIDIKAN_ID,
			   a.TAHUN TAHUN, 
			   a.JURUSAN JURUSAN, 
    ...
    function selectByParamsDataPendidikan($paramsArray=array(),$limit=-1,$from=-1, $statement='', $orderby='')
	{
		$str = "
		select * from simpeg.pendidikan
		where 1=1
		"; 
