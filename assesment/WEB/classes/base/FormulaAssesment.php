...
class FormulaAssesment extends Entity
{
	 ...
	 **/
	function selectByParams($paramsArray = array(), $limit = -1, $from = -1, $statement = '', $sOrder = "ORDER BY FORMULA_ID DESC")
	{
	...
	function selectByParamsNew($paramsArray = array(), $limit = -1, $from = -1, $statement = '', $sOrder = "ORDER BY a.FORMULA_ID DESC")
	{
		$str = "SELECT a.FORMULA_ID, FORMULA, a.TAHUN, a.KETERANGAN, TIPE_FORMULA, 
		CASE 
		WHEN TIPE_FORMULA = '1' THEN 'Tujuan Pengisian' 
		WHEN TIPE_FORMULA = '2' THEN 'Tujuan Pemetaan'
		WHEN TIPE_FORMULA = '3' THEN 'Uji Coba'
			...
			'-'
		END TIPE ,
		count(penilaian_detil_id) status_save
		FROM formula_assesment A 
		left join formula_eselon fe on a.formula_id = fe.formula_id
		left join penilaian_detil pd on fe.formula_eselon_id = pd.formula_eselon_id
		WHERE 1=1 ";
		...
		$str .= $statement . " group by a.FORMULA_ID, FORMULA, a.TAHUN, a.KETERANGAN, TIPE_FORMULA, TIPE_FORMULA " . $sOrder;
		$this->query = $str;
	...
	function selectByParamsMonitoringPegawai($paramsArray = array(), $limit = -1, $from = -1, $statement = '', $sOrder = "ORDER BY FORMULA_ID DESC")
	{
		$str = "
		SELECT
