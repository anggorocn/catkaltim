...
$set->selectByParamsAtributFormulaPenggalian(array(), -1,-1, $reqRowId, $statement);
// echo $set->query;exit;
while($set->nextRow())
{
	...
	$statement= " AND A.FORMULA_ATRIBUT_ID = ".$set->getField("FORMULA_ATRIBUT_ID");
	$set_atribut_pengalian= new AtributPenggalian();
	$set_atribut_pengalian->selectByParams(array(), -1,-1, $statement);
	// echo $set_atribut_pengalian->query;exit;
...
$statement= " AND A.PENGGALIAN_ID > 0 AND A.TAHUN = '".$tempFormulaTahun."'";
// $statement= " 1=2";
$index_loop= 0;
$arrPenggalian="";
$set= new Penggalian();
...
function setLoad()
{
	...
	document.location.href= 'formula_assesment_add_atribut.php?reqId=<?=$reqId?>&reqRowId=<?=$reqRowId?>&reqAspekId='+reqAspekId;
}
...
function setGeneralFilter(id)
{
	...
	?>
		...
		var reqAtributId= reqLevelValId= reqLevelId= reqFormulaAtributId= reqAtributParentLevelId= "";
		reqAtributId= $("#reqAtributId"+id).val();
		reqLevelId= $("#reqLevelId"+id).val();
		reqLevelValId= $("#reqLevelValId"+id).val();
		
		reqFormulaAtributId= $("#reqFormulaAtributId"+id).val();
		reqAtributParentLevelId= $("#reqAtributParentLevelId"+id).val();
		
		if(reqFormulaAtributId == ""){}
		else
		setChecked(id, 1);
		
		var s_url= "../json-pengaturan/<?=$arrReqJson[$indexReqFilter]?>.php?reqAtributId="+reqAtributId+"&reqFormulaAtributId="+reqFormulaAtributId+"&reqAtributParentLevelId="+reqAtributParentLevelId;
...
$(function(){
	...
	$('#ff').form({
		url:'../json-pengaturan/formula_assesment_add_atribut.php',
		onSubmit:function(){
...
function setChecked(id, val)
{
	...
	else
	{
		...
		var reqFormulaAtributId= reqAtributId= reqAtributPenggalianId= "";
		reqFormulaAtributId= $("#reqFormulaAtributId"+id).val();
		reqAtributId= $("#reqAtributId"+id).val();
		...
		//{
		?>
		//alert(reqFormulaAtributId);return false;
		if(reqFormulaAtributId == ""){}
		else
		{
			...
			//{
				$.messager.confirm('Confirm','Apakah anda yakin ingin menghapus data terpilih ?',function(r){
					if (r){
						var reqAspekId= "";
						reqAspekId= $("#reqAspekId").val();
						$.getJSON('../json-pengaturan/delete.php?reqMode=formula_atribut&id='+reqFormulaAtributId+"&reqAspekId="+reqAspekId, function (data) 
						{
...
<div id="bg"><img src="../WEB/images/wall-kanan-polos.png" width="100%" height="100%" alt=""></div>
    ...
    <tr>
        ...
        <th style="width:70%">Nilai standart</th>
        <th><th>
    ...
    <tr>
        ...
        <th style="width:5%">Nilai Standart</th>
        <th style="width:70%">Penggalian</th>
        <th></th>
        <th></th>
    ...
    ?>
        <tr>
            ...
            <input type="hidden" id="reqFormulaAtributBobotId<?=$checkbox_index?>" name="reqFormulaAtributBobotId[]" value="<?=$tempFormulaAtributBobotId?>" />
            ...
            <td>
                ...
                data-options="validType:'nilaiKurangSamaDengan[5]'" 
			...
			<?=$tempAtributParentNama?>
            <input type="hidden" id="reqFormulaAtributBobotId<?=$checkbox_index?>" name="reqFormulaAtributBobotId[]" value="" />
    ...
    ?>
    	...
    	<tr>
            ...
            <input type="hidden" name="reqAtributPenggalianId[]" id="reqAtributPenggalianId<?=$checkbox_index?>" value="<?=$tempAtributParentAtributPenggalianId?>" />
            <?
				$statement= " AND A.ATRIBUT_ID = '".$tempAtributParentAtributId."'";
				...
				if($jumlah_level_atribut > 0)
				{
                    ...
                    <!-- <?=$tempPenggalianKode?>&nbsp;&nbsp;&nbsp;<?=$tempPenggalianId?> -->
                    <?=$tempPenggalianKode?>&nbsp;&nbsp;&nbsp;
...
?>
	...
	var reqFormulaAtributId= "";
	reqFormulaAtributId= $("#reqFormulaAtributId<?=$checkbox_index?>").val();
	if(reqFormulaAtributId == ""){}
	else
