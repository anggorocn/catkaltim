...
$set->selectByParamsNew(array('A.PEGAWAI_ID'=>$reqId), -1, -1);
$set->firstRow();
// echo $set->query;exit;  
...
$set_catatan->selectByParams(array(), -1,-1, $statement_catatan);
// echo $set_catatan->query;exit;
$jumlahPenilaianPotensiProfilKompetensi=-1;
while($set_catatan->nextRow())
{
...
<div id="content" style="height:auto; width:100%;">
    ...
    <table class="table_list" cellspacing="1" width="95%" style="margin-bottom: 20px;">
        ...
        <tr >
            <td >Ringkasan</td>
            <td>:</td>
            <td style="text-align: justify; ">Berdasarkan  hasil    penilaian kompetensi, menunjukan bahwa nilai total kompetensi Saudara <?=$infopegawainama?> adalah <?=$nilaiIndividu?> dari total <?=$nilaiStandar?> atau setara dengan <?=$jpm?>% Job Person Match (JPM)
            <? 
            for($index_catatan=0; $index_catatan<$jumlahPenilaianPotensiProfilKompetensi; $index_catatan++)
            {
            ?>
                <p><?=nl2br($reqinfocatatan)?></p>
            <?
            }
            ?>
        </td>
        ...
        <tr >
            ...
            <td style="text-align: justify; ">
                ...
                ?>
                <p  style="font-size: 10pt;text-align: justify;"><?=nl2br($reqinfocatatan);?></p>
                <?
                }
                ?>
            </td>
        ...
        <tr>
            <td >Rencana Pengambangan Diri</td>
            <td>:</td>
            <td style="text-align: justify; ">
               <? 
                $set= new PegawaiHcdp();
                $set->selectByParamsDetilFeedback(array("f.PEGAWAI_ID"=>$reqId,"f.JADWAL_TES_ID"=>$reqJadwalTesId), -1,-1);
                // echo $set->query;exit;
                while($set->nextRow())
                {         
                    ?>
                    <p  style="font-size: 10pt;text-align: justify;"><?=nl2br($set->getField('Keterangan'))?></p>
                    <?
                }
                ?>
            </td>
