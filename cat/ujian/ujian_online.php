...
$tempInfoJudul= "Ujian ".$tempKeterangan;
...
if($tempTipeUjianId == "7")
{
	...
	while($set->nextRow())
	{
		...
		$arrJumlahJawabanSoalPegawai[$index_loop]["JAWABAN"]=ucfirst($set->getField("JAWABAN"));
...
elseif($tempTipeUjianId == "17")
{
	...
	while($set->nextRow())
	{
		...
		else
		{
			...
			$arrJumlahSoalPegawai[$index_loop]["KEMAMPUAN"]= $set->getField("KEMAMPUAN");
			$arrJumlahSoalPegawai[$index_loop]["KATEGORI"]= $set->getField("KATEGORI");
			$arrJumlahSoalPegawai[$index_loop]["PERTANYAAN"]= ucfirst($set->getField("PERTANYAAN"));
			$arrJumlahSoalPegawai[$index_loop]["TIPE_SOAL"]= $set->getField("TIPE_SOAL");
			$arrJumlahSoalPegawai[$index_loop]["PATH_GAMBAR"]= $set->getField("PATH_GAMBAR");
			$arrJumlahSoalPegawai[$index_loop]["PATH_SOAL"]= $set->getField("PATH_SOAL");
...
elseif($tempTipeUjianId == "45")
{
    ...
    $statementujian= " AND A.PEGAWAI_ID = ".$tempPegawaiId." AND A.UJIAN_ID = ".$tempUjianId." AND A.UJIAN_TAHAP_ID = ".$reqId;
	$set= new UjianPegawaiDaftar();
	$set->selectByParamsSoalBigFive(array(), -1,-1, $tempUjianPegawaiLowonganId, $statement, $statementujian, $sOrder);
	// echo $set->query;exit;
	$tempValueRowId= "";
	while($set->nextRow())
	{
	...
	$sOrder= "ORDER BY A.UJIAN_ID, B.BANK_SOAL_ID, D.JAWABAN";
	$index_loop=0;
	$arrJumlahJawabanSoalPegawai=array();
    ...
    $set= new UjianPegawaiDaftar();
	$set->selectByParamsJawabanSoalBigFive(array(), -1,-1, $statement, $sOrder);
	// echo $set->query;exit;
	while($set->nextRow())
	{
		$arrJumlahJawabanSoalPegawai[$index_loop]["ID_ROW"]= $set->getField("UJIAN_ID")."-".$set->getField("BANK_SOAL_ID");
...
elseif($tempTipeUjianId == "47")
{
    ...
    $statementujian= " AND A.PEGAWAI_ID = ".$tempPegawaiId." AND A.UJIAN_ID = ".$tempUjianId." AND A.UJIAN_TAHAP_ID = ".$reqId;
	$set= new UjianPegawaiDaftar();
	$set->selectByParamsSoalWpt(array(), -1,-1, $tempUjianPegawaiLowonganId, $statement, $statementujian, $sOrder);
	// echo $set->query;exit;
	$tempValueRowId= "";
	while($set->nextRow())
	{
		...
		else
		{
			...
			$arrJumlahSoalPegawai[$index_loop]["PATH_GAMBAR"]= $set->getField("PATH_GAMBAR");
			$arrJumlahSoalPegawai[$index_loop]["PATH_SOAL"]= $set->getField("PATH_SOAL");
			$arrJumlahSoalPegawai[$index_loop]["JENIS"]= $set->getField("JENIS");
			$index_loop++;
		}
		$tempValueRowId= $set->getField("UJIAN_ID")."-".$set->getField("BANK_SOAL_ID");
	...
	$sOrder= "ORDER BY A.UJIAN_ID, B.BANK_SOAL_ID, D.WPT_PILIHAN_ID, D.JAWABAN";
	$index_loop=0;
	$arrJumlahJawabanSoalPegawai=array();
    ...
    $set= new UjianPegawaiDaftar();
	$set->selectByParamsJawabanSoalWpt(array(), -1,-1, $statement, $sOrder);
	// echo $set->query;exit;
	while($set->nextRow())
	{
		$arrJumlahJawabanSoalPegawai[$index_loop]["ID_ROW"]= $set->getField("UJIAN_ID")."-".$set->getField("BANK_SOAL_ID");
...
elseif($tempTipeUjianId == "48")
{
    ...
    $statementujian= " AND A.PEGAWAI_ID = ".$tempPegawaiId." AND A.UJIAN_ID = ".$tempUjianId." AND A.UJIAN_TAHAP_ID = ".$reqId;
	$set= new UjianPegawaiDaftar();
	$set->selectByParamsSoalKertih(array(), -1,-1, $tempUjianPegawaiLowonganId, $statement, $statementujian, $sOrder);
	// echo $set->query;exit;
	$tempValueRowId= "";
	while($set->nextRow())
	{
    ...
    $set= new UjianPegawaiDaftar();
	$set->selectByParamsJawabanSoalKertih(array(), -1,-1, $statement, $sOrder);
	// echo $set->query;exit;
	while($set->nextRow())
	{
		$arrJumlahJawabanSoalPegawai[$index_loop]["ID_ROW"]= $set->getField("UJIAN_ID")."-".$set->getField("BANK_SOAL_ID");
...
elseif($tempTipeUjianId == "49")
{
    ...
    $statementujian= " AND A.PEGAWAI_ID = ".$tempPegawaiId." AND A.UJIAN_ID = ".$tempUjianId." AND A.UJIAN_TAHAP_ID = ".$reqId;
	$set= new UjianPegawaiDaftar();
	$set->selectByParamsSoalHolland(array(), -1,-1, $tempUjianPegawaiLowonganId, $statement, $statementujian, $sOrder);
	// echo $set->query;exit;
	$tempValueRowId= "";
	while($set->nextRow())
	{
    ...
    $set= new UjianPegawaiDaftar();
	$set->selectByParamsJawabanSoalHolland(array(), -1,-1, $statement, $sOrder);
	// echo $set->query;exit;
	while($set->nextRow())
	{
		$arrJumlahJawabanSoalPegawai[$index_loop]["ID_ROW"]= $set->getField("UJIAN_ID")."-".$set->getField("BANK_SOAL_ID");
...
else
{
	...
	// kondisi ist:10, TKD 2 R:70, TKD 4:71, TKD 5R:72,TKD 6R:73,TKD 10:74
	if($tempTipeUjianId == 40 || $tempParentId == "10" || $tempTipeUjianId == 70 || $tempTipeUjianId == 71  || $tempTipeUjianId == 72 || $tempTipeUjianId == 73 || $tempTipeUjianId == 74)
	{
		$sOrder= "ORDER BY A.UJIAN_ID, B.BANK_SOAL_ID, UP.UJIAN_PEGAWAI_ID";
	}
	...
	if( ( ($tempParentId == "10" || $tempParentId == "12") && $tempPanjangTipeUjianId == 2) || ($tempTipeUjianId == 72 || $tempTipeUjianId == 73) )
	{
		if($tempTipeUjianId == 35 || $tempTipeUjianId == 73)
		{
			$index_loop=0;
			$arrKeterangan=array();
			$statement= " AND A.PEGAWAI_ID = ".$tempPegawaiId." AND A.UJIAN_TAHAP_ID = ".$reqId." AND A.UJIAN_ID = ".$tempUjianId;
...
<script language="javascript">
	...
	$(function(){
		...
		$('button[id^="reqTextWptButton"]').click(function(e) {
			var tempId= $(this).attr('id');


			var tempValId= $(this).val();
			tempId= tempId.split('reqTextWptButton');
			...
			reqKeterangan= $("#reqWptText"+tempPilihRowId).val();
		...
		$('button[id^="reqTextKertihButton"]').click(function(e) {
			var tempId= $(this).attr('id');


			var tempValId= $(this).val();
			tempId= tempId.split('reqTextKertihButton');
			tempId= tempId[1].split('-');
			var tempNomor= tempUjianId= tempUjianBankSoalId= tempBankSoalId= tempBankSoalPilihanId= "";
			tempNomor= tempId[0];
			tempUjianId= tempId[1];
			...
			reqKeterangan= $("#reqKertihText"+tempPilihRowId).val();
			// console.log(reqKeterangan); return false;
			...
			$.ajax({'url': s_url,'success': function(msg) {
				if(msg == '')
				{
					$("#reqKertihText"+tempPilihRowId).val("");
					$.messager.alert('Info', "Data gagal disimpan, karena nilai "+reqKeterangan+". Data bisa disimpan kalau data 0 sampai 5.", 'info');
					$.messager.progress('close');
				}
				else
				{
				...
				//{
					...
					else
					{
						...
						(
							...
							$tempTipeUjianId == 28 || $tempTipeUjianId == 46 ||
							($tempTipeUjianId >= 30 && $tempTipeUjianId <= 39) ||
							$tempTipeUjianId == 40 || $tempTipeUjianId == 41 || $tempTipeUjianId == 42 || $tempTipeUjianId == 45 || $tempTipeUjianId == 48 || $tempTipeUjianId == 49 || $tempTipeUjianId >= 50
						)
						{
						?>
							// kondisicheck= "";
	...
	<div class="row">
    	...
    	<div class="col-md-12">
			<div class="area-judul-halaman">
            	<?=$tempInfoJudul?>
            	<span style="float:right; display: none;" class="lengkapimodif-data" id="reqIdSelesai"><a href="#" onclick="setFinish()" class="selesai">Selesai &raquo;</a></span>
            </div>
        </div>
                ...
                <div id="main-slider" class="liquid-slider">
					...
					for($index_loop=0; $index_loop < $tempJumlahSoalPegawai; $index_loop++)
					{
						...
						else
						{
							...
							$infobanksoaljenis= $arrJumlahSoalPegawai[$index_loop]["JENIS"];
						}
					...
					?>
                    	<div id="<?=$tempNomor?>" class="area-soal-item">
							...
							?>
								<?
								$arrexcept= array("16","38","40","42","49","7");
								if(in_array($tempBankSoalId, $arrexcept))
								{
								?>
	                                <span class="teks"><?=$tempPertanyaan?></span>
									<span class="gambar-soal-kiri">
	                                <?
	                                $urllinkfile= "../main/uploads/wpt/soal".$tempBankSoalId.".png";
									if(file_exists($urllinkfile))
									{
	                                ?>
	                                <img src="<?=$urllinkfile?>">
	                                <?
							...
							elseif($tempTipeUjianId==48 || $tempTipeUjianId==49)
							{
							?>
								<span class="pertanyaan"><?=$tempPertanyaan?></span>
							<?
							}
							else if($tempTipeSoal==1 || $tempTipeSoal==8 || $tempTipeSoal==9 || $tempTipeSoal==10 || $tempTipeSoal=="6x6" || $tempTipeSoal==45 || $tempTipeSoal==47)
							{
                            ?>
                        		<span class="pertanyaan"><?=$tempPertanyaan?></span>
                            <?
							...
							{
								// if($reqPegawaiId == 1574 && $nomor == 40)
								if($tempPegawaiId == 225)
								{
									// print_r($arrayJawaban);
									// print_r($arrayJawaban1);
									// exit();
								}
							...
							?>
                                ...
                                if($tempTipeUjianId==47)
								{
									$arrexceptgambar= array("7");
									$arrexcept= array("23","38","41","42","49");
									if($infobanksoaljenis == "2" || $infobanksoaljenis == "3")
									{
										$reqKeterangan= "";
										$arrayJawaban= [];
			                            $arrayJawaban= in_array_column($tempIdRow, "ID_ROW", $arrKeterangan);
								...
								?>
										<!-- <input type="text" class="easyui-validatebox" style="width: 100%;" name="reqWptText<?=$tempUjianId?>-<?=$tempUjianBankSoalId?>-<?=$tempBankSoalId?>" id="reqWptText<?=$tempNomor?>-<?=$tempUjianId?>-<?=$tempUjianBankSoalId?>-<?=$tempBankSoalId?>" value="<?=$reqKeterangan?>" /> -->
										<textarea class="easyui-validatebox" rows="4" style="width: 100%;" name="reqWptText<?=$tempUjianId?>-<?=$tempUjianBankSoalId?>-<?=$tempBankSoalId?>" id="reqWptText<?=$tempNomor?>-<?=$tempUjianId?>-<?=$tempUjianBankSoalId?>-<?=$tempBankSoalId?>"><?=$reqKeterangan?></textarea>
                            			<button id="reqTextWptButton<?=$tempNomor?>-<?=$tempUjianId?>-<?=$tempUjianBankSoalId?>-<?=$tempBankSoalId?>">Simpan</button>
								<?
									}
									elseif(in_array($tempBankSoalId, $arrexcept))
									{
										if ($tempBankSoalId!="23" and $tempBankSoalId!="41"){?><div class="col-md-2"><?}?>
												<label class="pilihan-ganda" for="reqRadio<?=$tempNomor?>-<?=$tempUjianId?>-<?=$tempUjianBankSoalId?>-<?=$tempBankSoalId?>-<?=$tempBankSoalPilihanDetilId?>">
			                                        <input type="checkbox" class="easyui-validatebox" name="reqRadio<?=$tempUjianId?>-<?=$tempUjianBankSoalId?>-<?=$tempBankSoalId?>" id="reqRadio<?=$tempNomor?>-<?=$tempUjianId?>-<?=$tempUjianBankSoalId?>-<?=$tempBankSoalId?>-<?=$tempBankSoalPilihanDetilId?>" value="<?=$tempBankSoalPilihanDetilId?>" />
			                                        <span>	                                        	
														<?=$tempJawaban?>	
													</span>
			                                    </label>
										<?if ($tempBankSoalId!="23" and $tempBankSoalId!="41"){?></div><?}?>
	                                    
								<?
									}
									elseif(in_array($tempBankSoalId, $arrexceptgambar))
									{
										$urllinkfile= "../main/uploads/wpt/".$tempJawaban;
								?>
										<label class="pilihan-ganda" for="reqRadio<?=$tempNomor?>-<?=$tempUjianId?>-<?=$tempUjianBankSoalId?>-<?=$tempBankSoalId?>-<?=$tempBankSoalPilihanDetilId?>">
											...
											<?
											if($gambar==0){
												$urllinkfile= "../main/uploads/wpt/soal".$tempBankSoalId."a.png";
											}
											else if($gambar==1){
												$urllinkfile= "../main/uploads/wpt/soal".$tempBankSoalId."b.png";
											}
											else if($gambar==2){
												$urllinkfile= "../main/uploads/wpt/soal".$tempBankSoalId."c.png";
											}
											else if($gambar==3){
												$urllinkfile= "../main/uploads/wpt/soal".$tempBankSoalId."d.png";
											}
											else if($gambar==4){
												$urllinkfile= "../main/uploads/wpt/soal".$tempBankSoalId."e.png";
											}
											$gambar++;
											...
											{
											?>
	                                        <img src="<?=$urllinkfile?>" height="100" />
											<?
											}
	                                        ?>
											</span>
								...
								?>
									<input type="text" class="easyui-validatebox" name="reqKertihText<?=$tempUjianId?>-<?=$tempUjianBankSoalId?>-<?=$tempBankSoalId?>" id="reqKertihText<?=$tempNomor?>-<?=$tempUjianId?>-<?=$tempUjianBankSoalId?>-<?=$tempBankSoalId?>" value="<?=$reqKeterangan?>" />
                            		<button id="reqTextKertihButton<?=$tempNomor?>-<?=$tempUjianId?>-<?=$tempUjianBankSoalId?>-<?=$tempBankSoalId?>">Simpan</button>
								<?
								}
								elseif($tempTipeSoal==1 || $tempTipeSoal==5 || $tempTipeUjianId == 39 || $tempTipeUjianId == 45)
								{
									if($tempJawaban == "")
									continue;
								?>
								...
								{
	                               if($tempTipeUjianId==23 || $tempTipeUjianId==24)
									{
	                                ?>
	                                	<label class="gambar-kecil" style="height:50px; width: 20%;padding-top: 20px;">
								...
								elseif($tempTipeSoal==7 || $tempTipeSoal==17 || $tempTipeSoal==49)
								{
									if($tempJawaban == "")
									continue;
								?>
    	...
    	<div class="area-prev-next-ujian reqInfoIsi" style="margin-top: 40px">
        	<?php /*?><div class="selesai" style="display:none" id="reqIdSelesai"><a href="?pg=finish">Selesai &raquo;</a></div><?php */?>
        	<div class="col-md-10">

	        	<div class="col-md-6">
	            	<div class="prev-ujian" style="float: right;" id="reqIdPrev">
	            		<a href="#" id="reqHrefIdPrev" style="background-color:  #ff5733 ;" onclick="setPrev()"><i style="background-color:  #ff5733"  class="fa fa-chevron-left"></i>  Sebelumnya</a>
	            	</div>
	        		
	        	</div>
	        	<div class="col-md-6">
	            	<div class="next-ujian" style="float: left;" id="reqIdNext"><a style="background-color: #33bbff; color: black;  "  href="javascript:void(0);" id="reqHrefIdNext" onclick="setNext()"> Selanjutnya <i style="background-color:   #33bbff "  class="fa fa-chevron-right"></i></a></div>
...
$('input[id^="reqWptText"]').bind('keyup paste', function(){
	var tempId= $(this).attr('id');

	var tempValId= $(this).val();
	tempId= tempId.split('reqWptText');
	tempId= tempId[1].split('-');
	tempNomor= tempId[0];

	if(tempNomor == 17 || tempNomor == 27 || tempNomor == 49){}
	else
	this.value = this.value.replace(/[^0-9\./]/g, '');
});

$('input[id^="reqKertihText"]').bind('keyup paste', function(){
	var tempId= $(this).attr('id');

	var tempValId= $(this).val();
	tempId= tempId.split('reqKertihText');
	tempId= tempId[1].split('-');
	tempNomor= tempId[0];
